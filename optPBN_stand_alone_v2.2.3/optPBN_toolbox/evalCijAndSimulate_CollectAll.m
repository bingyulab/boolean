function [cijs,MeanStateValueAll,StdStateValueAll,MeanCostAll,StdCostAll] = evalCijAndSimulate_CollectAll(estim,bestRun,simFlag,x)
% cijs = evalCijAndSimulate_CollectAll(estim,bestRun,simFlag,x)
...
% Note: The results from the simulations might not always be the same but are closely clustered (based on accuracy parameter)
%
...
if ~isempty(bestRun)
    x=history(bestRun,(end-estim.paramNr+1):end);
    disp(['Run Nr ' num2str(bestRun) ' used.'])
end
...
for counter3=1:estim.NrExps
    ...
    if simFlag
        ...
        for iteration=1:10
            ...
            pdf_ic_mean_ALL=[pdf_ic_mean_ALL; pdf_ic_mean];
            ...
            diff=acc_diff-temp_diff;
            diff_ALL=[diff_ALL; diff];
        end
                
        MeanAllState=mean(pdf_ic_mean_ALL);
        StdAllState=std(pdf_ic_mean_ALL);
 
        MeanStateValueAll=[MeanStateValueAll; MeanAllState];
        StdStateValueAll=[StdStateValueAll; StdAllState];
        MeanCostAll=[MeanCostAll; acc_diff/iteration(end)];
        StdCostAll=[StdCostAll; std(diff_ALL)];

...
compare_attractors <- function(original_attractors, modified_attractors) {
  # Get column names (signals) for each
  orig_names <- colnames(original_attractors[[1]])
  mod_names <- colnames(modified_attractors[[1]])
  all_names <- union(orig_names, mod_names)
  
  # Convert attractors to matrices with all columns, fill missing with 0
  orig_mat <- t(sapply(original_attractors, function(x) {
    v <- rep(0, length(all_names))
    names(v) <- all_names
    v[names(x)] <- as.numeric(x)
    v
  }))
  mod_mat <- t(sapply(modified_attractors, function(x) {
    v <- rep(0, length(all_names))
    names(v) <- all_names
    v[names(x)] <- as.numeric(x)
    v
  ...
  # Ensure same number of attractors
  n <- min(nrow(orig_mat), nrow(mod_mat))
  orig_mat <- orig_mat[1:n, , drop=FALSE]
  mod_mat <- mod_mat[1:n, , drop=FALSE]
  
  # Hamming distance (normalized)
  hamming <- rowSums(orig_mat != mod_mat)
  norm_hamming_orig <- hamming / ncol(orig_mat)
  norm_hamming_max <- hamming / length(all_names)
  
  # Precision, recall, F1, accuracy for each attractor
  metrics <- t(sapply(1:n, function(i) {
    tp <- sum(orig_mat[i, ] == 1 & mod_mat[i, ] == 1)
    fp <- sum(orig_mat[i, ] == 0 & mod_mat[i, ] == 1)
    fn <- sum(orig_mat[i, ] == 1 & mod_mat[i, ] == 0)
    tn <- sum(orig_mat[i, ] == 0 & mod_mat[i, ] == 0)
...
compareNetwork <- function(origFile, modifiedFiles, bodySeparator = ",", compareAttractors=TRUE, compareTransitionTables=FALSE)
{
  #source("../functionClassAnalysis.R")
  origNet <- loadNetwork(origFile, symbolic=TRUE)
  ...
  modifiedNets <- lapply(modifiedFiles, function(file)loadNetwork(file, bodySeparator=bodySeparator, symbolic=TRUE)) 
  
  modAtt <- lapply(modifiedNets, function(net)
                    getAttractors(net, startStates=startStates))
  sink()
  # attractorsEqual <- sapply(modAtt, function(att) compare_attractors(origAtt$attractors, att$attractors))
  
  attractorsEqual <- lapply(seq_along(modAtt), function(i) {
    df <- compare_attractors(origAtt$attractors, modAtt[[i]]$attractors)
    rownames(df) <- paste0("Attractor_", df$attractor)
    df$attractor <- NULL
    return(df)
  })
  names(attractorsEqual) <- basename(unlist(modifiedFiles))

  # Optionally print a summary for each comparison
  for (name in names(attractorsEqual)) {
    cat("\nComparison for:", name, "\n")
    print(attractorsEqual[[name]])
  }

  # Return comparison results as a list
  return(attractorsEqual)
}

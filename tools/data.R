...
remove_self_loops <- function(bnet_file, tmp_file = tempfile(fileext = ".bnet")) {
  # read bnet file (assume CSV with header: targets, factors)
  bnet <- read.csv(bnet_file, header = TRUE, stringsAsFactors = FALSE, strip.white = TRUE)
  
  # keep rows where target != factor
  cleaned <- subset(bnet, targets != factors)
  
  # write to tmp file
  write.table(cleaned, file = tmp_file, sep = ",", row.names = FALSE, quote = FALSE)
  
  message("Cleaned bnet file saved to: ", tmp_file)
  return(tmp_file)
}


convertSBML <- function(sbml_file) {
  library(BoolNet)
  # Load the SBML model
  model <- loadSBML(sbml_file)
  
  # Convert the model to BoolNet format
